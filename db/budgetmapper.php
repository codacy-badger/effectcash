...
class BudgetMapper extends Mapper {
    ...
    public function findBetween($id, $userId, $start, $end) {
      $sql = 'SELECT * FROM *PREFIX*effectcash_budgets WHERE user_id = ?'
        . 'AND (DATE(`date`) BETWEEN ? AND ?)'
        . 'OR ((LEFT(`repeat`, 1) = "m" OR LEFT(`repeat`, 1) = "w" OR LEFT(`repeat`, 1) = "y") AND EXTRACT(YEAR_MONTH FROM `date`) <= EXTRACT(YEAR_MONTH FROM NOW()))';
